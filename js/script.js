...
const filtrarportipo = document.getElementById("filtrarportipo")
const mostrarportipo= document.getElementById("mostrarportipo")
const btnfiltrar= document.getElementById("btnfiltrar")
btnfiltrar.addEventListener("click",()=>{
  mostrarportipo.innerHTML=""; 
 let arreglo= ordenarPorType(filtrarportipo.value);
 for (let i = 0; i < arreglo.length; i++) {
  for (let atributo in arreglo[i]) {
    mostrarportipo.innerHTML += atributo + ': ' +arreglo[i][atributo]+ '<br>';
  }
  mostrarportipo.innerHTML += '<br>';
 }
}) 
